...
import TaintMemorySQL.{columnsAreTainted, isTainted, taintTableAndColums}
...
object SqlStringTaintAnalyzer{
  ...
  def analyzeInsertStatement(normalizedString:String) = {
    ...
    var taintedColumns: Set[String] = columns.filter(column => isTainted(column)).toSet

    valueGroups.foreach(array => {
      for(i <- array.indices){
        if(isTainted(array(i))) taintedColumns += columns(i)
      }
    })

    if(isTainted(table)) taintedColumns ++=columns
    if(taintedColumns.nonEmpty) taintTableAndColums(table,taintedColumns)
  ...
  def analyzeSelectStatment(normalizedString: String) ={
    ...
    if(columns.contains("*")){
      TaintMemorySQL.taintedTableAndColumns.get(table) match {
        case Some(x) => tempColumn = x
        case None =>
      }
    }

    if(isTainted(table)) taintTableAndColums(table,tempColumn)
    val (hastaints,taintedColumns) = columnsAreTainted(table,tempColumn)
  ...
  def analyzeUpdateStatment(normalizedString: String)= {
    ...
    for((colmn,value) <-columnAndValues){
      if(isTainted(value)||isTainted(colmn)||isTainted(table)){
        taintColumns += colmn
      }
    }

    if(taintColumns.nonEmpty) taintTableAndColums(table,taintColumns)
